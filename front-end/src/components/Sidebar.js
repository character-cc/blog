...
const Sidebar = () => {
    ...
    useEffect(() => {
        ...
        const getUser = async () => {
           const response = await fetchWrap("http://localhost/api/users/me/follows" );
            if(response.ok){
        ...
        //     const response = await fetchWrap("http://localhost/api/posts/follow" )
        //     if(response.ok){
        //         const data = await response.json();
        //         console.log(data);
        //         if(data){
    ...
    const handleClickButton = (id) =>{
        const followingUser = async () => {
            const response = await fetchWrap("http://localhost/api/users/me/follows/"+id , {method: "POST"});
            if(response.ok){
    ...
    return (
        <div className="w-25" style={{borderLeft: "solid 1px #5c5a5a"}}>
            ...
            {isAuthenticated ? (<>
                ...
                {/*            <Link to={`/posts/${post.id}`} style={{textDecoration : "none" , color: "black"}} ><h5 style={{cursor : "pointer"}}>{post.title}</h5></Link>*/}
                {/*        </div>*/}
                {/*        <div className="row">*/}
                ...
                <div className="col"><h5>Bạn cần đăng nhập</h5></div>  </div>)}
        </div>
    );
};

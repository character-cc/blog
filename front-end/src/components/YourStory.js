...
const YourStory = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                const url = "http://localhost/api/users/me/story";
    ...
    const handleMoreClick = () => {
        const getMore = async () => {
            try {
                const response = await fetchWrap("http://localhost/api/users/me/story?page=" + count);
                if (response.ok) {
    ...
    const handleDeleteClick = (postId) => {
        const deletePost = async () => {
            try {
                ...
                if (isConfirmed) {
                    const url = "http://localhost/api/posts/" + postId;
                    const response = await fetchWrap(url, {method: "DELETE"});
                    if (response.ok) {
                        setPosts((prevPosts) => {
                ...
                <div className="container">
                    <div className="ms-5 mt-5">
                        <div className="mx-5">
                            ...
                            {end ? <div className="d-flex justify-content-center mt-5">
                                    <div style={{cursor: "pointer", color: "green"}} onClick={handleMoreClick}><h5>Bạn đã xem hết</h5></div>
                                </div>
